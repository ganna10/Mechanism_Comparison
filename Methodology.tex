...
Differences in \ce{O3} production between the CRI v2 and MCM v3.2 may be due to changes in the MCM versions rather than the CRI reduction techniques, hence the \mbox{MCM v3.1} is also included in this study.
...
These \ce{NO_x} conditions induce \ce{NO_x}-VOC-sensitive chemistry which would be lower than the \ce{NO_x}-saturated conditions found in Los Angeles.
Ambient \ce{NO_x} conditions are not required as this study is concerned with the maximum potential of VOC to produce \ce{O3}.
Future work could verify the extent to which the maximum potential is reached under ambient \ce{NO_x} conditions.
...
Methane (\ce{CH4}) was fixed at \mbox{$1.8$ ppmv} while CO and \ce{O3} were initialised at \mbox{$200$ ppbv} and \mbox{$40$ ppbv} and then allowed to evolve freely.

The VOC used in this study are assigned to mechanism species following the recommendations from the literature of each mechanism (Table \ref{t:mechanisms}), the representation of each VOC in the mechanisms is found in \mbox{Table \ref{t:initial_conditions}}.
...
The MCM v3.2 approach to photolysis, dry deposition of VOC oxidation intermediates and \ce{RO2}--\ce{RO2} reactions was used for each mechanism; details of these adaptations can be found in the online supplement to this paper.
Some mechanisms include reactions which are only important in the stratosphere or free troposphere.
For example, PAN photolysis is only important in the free troposphere \citep{Harwood:2003} and was removed from MOZART-4, RACM2 and CB05 for the purpose of the study, as this study considers processes occurring below the planetary boundary layer.
...
Thus, production of \ce{O_x} can be used as a proxy for production of \ce{O3}.
...
\citet{Butler:2011} showed that \ce{O_x} production can be attributed to the VOC by following the tags of each VOC.
...
Many reduced mechanisms use an operator species as a surrogate for \ce{RO2} during VOC degradation enabling these mechanisms to produce \ce{O_x} while minimising the number of \ce{RO2} species.
