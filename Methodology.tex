...
VOC degradation is simplified by lumping the degradation products of many VOC into mechanism species whose \ce{O3} production reflects that of the \mbox{MCM v3.1}. 
The full version of the \mbox{CRI v2} (\url{http://mcm.leeds.ac.uk/CRI}) is used in this study.
Differences in \ce{O3} production between the CRI v2 and MCM v3.2 may be due to changes in the MCM versions rather than the CRI reduction techniques, hence the MCM v3.1 is included.
...
Thus an initial mixing ratio of $1200$ pptv would be assigned to \mbox{$6000$ ($= 1200 \times 5$) pptv} in CBM-IV.
...
We define the \ce{O_x} family to include \ce{O3}, \ce{O(^3P)}, \ce{O(^1D)}, \ce{NO2} and other species involved in fast cycling with \ce{NO2}, such as \ce{HO2NO2} and PAN species.
\citet{Butler:2011} shows that \ce{O_x} and \ce{O3} production are closely related.
...
%    \end{tikzpicture} 
%    \vspace{1mm}
%    \caption{The organic species, \ce{CH3O2}, HCHO and CO, are formed during \ce{CH4} degradation and are re-written with a CH4 tag, enabling attribution of \ce{O3} production to \ce{CH4}.}
%    \vspace{-4mm}
%    \label{f:tagging_approach}
%\end{figure} 

The \ce{O_x} production budget is allocated to individual VOC by following the VOC tags of each VOC \mbox{(Figure \ref{f:Ox_budget} (a)).}
Without tagging, \ce{O_x} production can only be attributed to the individual reactions \mbox{(Figure \ref{f:Ox_budget} (b))}, lacking information about the VOC source of the organic reactants.

\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{img/MCMv3_2_tagged_non_tagged_Ox_budget}
    \vspace{2mm}
    \caption{\ce{O_x} production budgets using the MCM v3.2, \mbox{(a) with} tagging, where \ce{O_x} production is attributed to individual VOC and (b) without tagging.}
    \vspace{-4mm}
    \label{f:Ox_budget}
\end{figure} 

\subsubsection{Tagged Ozone Production Potential (TOPP) Definition} \label{sss:TOPP} %final definition of TOPP

Allocating \ce{O_x} production to individual VOC using the tagging approach is the basis for calculating the TOPP of a VOC.
The TOPP of a VOC is the number of \ce{O_x} molecules produced per emitted VOC molecule and is defined as the daily contribution to \ce{O_x} production from a VOC normalised by the total emissions of the VOC.
...
For example, CB05 represents hexane as $6$ PAR and the TOPP value of hexane in the CB05 is $6$ times the TOPP of PAR.
Whilst in MOZART-4, hexane is represented by the five carbon species BIGALK.
Thus hexane emissions are scaled by $\frac{5}{6}$ and the TOPP value of hexane in MOZART-4 is calculated by multiplying the TOPP value of BIGALK by $\frac{6}{5}$.
